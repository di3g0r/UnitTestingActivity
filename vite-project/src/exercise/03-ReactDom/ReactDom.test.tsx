...
test('counter increments and decrements when the buttons are clicked', () => {
  ...
  const messageElement = div.querySelector('h1');
  
  expect(messageElement.textContent).toBe('Counter: 0');
  
  act(() => {
    increment.click();
  });
  expect(messageElement.textContent).toBe('Counter: 1');
  
  act(() => { 
    increment.click();
  });
  expect(messageElement.textContent).toBe('Counter: 2');
